...
/**
 * Renders the Electric Skeleton on the shared vanilla-skeleton rig ({@link SkeletonWarriorModel},
 * electric recolor texture) with full-brightness cyan-white eyes via the reusable {@link GMMEyesLayer}.
 * Armor layers reuse the skeleton armor models (Electric spawns unarmored by default, but the layer is
 * kept for parity with the other skeleton variants + any picked-up gear).
 *
 * @author Mark Gottschling on 7/4/2026
 */
public class ElectricSkeletonRenderer<T extends ElectricSkeleton> extends HumanoidMobRenderer<T, SkeletonWarriorModel<T>> {
